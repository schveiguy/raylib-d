...
void main()
{
    ...
    Rectangle[MAX_RECS_X*MAX_RECS_Y] recs;
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (state == 0)
        {
            ...
            for (int i = 0; i < MAX_RECS_X*MAX_RECS_Y; i++)
            {
                ...
                rotation = EaseLinearIn(cast(float)framesCounter, 0.0f, 360.0f, PLAY_TIME_IN_FRAMES);
            }
        }
        ...
        BeginDrawing();
            ...
            if (state == 0)
            {
                for (int i = 0; i < MAX_RECS_X*MAX_RECS_Y; i++)
                {
                    DrawRectanglePro(recs[i], Vector2( recs[i].width/2, recs[i].height/2 ), rotation, Colors.RED);
                }
            }
            else if (state == 1) DrawText("PRESS [SPACE] TO PLAY AGAIN!", 240, 200, 20, Colors.GRAY);
