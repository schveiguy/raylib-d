...
void main()
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (CheckCollisionPointRec(mousePosition, Rectangle( rec.x + rec.width - MOUSE_SCALE_MARK_SIZE, rec.y + rec.height - MOUSE_SCALE_MARK_SIZE, MOUSE_SCALE_MARK_SIZE, MOUSE_SCALE_MARK_SIZE )))
        {
            ...
            if (IsMouseButtonPressed(MouseButton.MOUSE_BUTTON_LEFT)) mouseScaleMode = true;
        }
        ...
        if (mouseScaleMode)
        {
            ...
            if (IsMouseButtonReleased(MouseButton.MOUSE_BUTTON_LEFT)) mouseScaleMode = false;
        }
        //----------------------------------------------------------------------------------
        ...
        BeginDrawing();
            ...
            DrawText("Scale rectangle dragging from bottom-right corner!", 10, 10, 20, Colors.GRAY);

            DrawRectangleRec(rec, Fade(Colors.GREEN, 0.5f));

            if (mouseScaleReady)
            {
                DrawRectangleLinesEx(rec, 1, Colors.RED);
                DrawTriangle(Vector2( rec.x + rec.width - MOUSE_SCALE_MARK_SIZE, rec.y + rec.height ),
                             Vector2( rec.x + rec.width, rec.y + rec.height ),
                             Vector2( rec.x + rec.width, rec.y + rec.height - MOUSE_SCALE_MARK_SIZE ), Colors.RED);
            }
