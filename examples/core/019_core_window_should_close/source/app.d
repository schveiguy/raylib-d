...
void main()
{
    ...
    while (!exitWindow)
    {
        ...
        BeginDrawing();
            ...
            if (exitWindowRequested)
            {
                DrawRectangle(0, 100, screenWidth, 200, Colors.BLACK);
                DrawText("Are you sure you want to exit program? [Y/N]", 40, 180, 30, Colors.WHITE);
            }
            else DrawText("Try to close the window to get confirmation message!", 120, 200, 20, Colors.LIGHTGRAY);
